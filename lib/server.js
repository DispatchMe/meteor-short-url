...
UrlShortener = class UrlShortener {
  ...
  createShortUrl(url, {
    ...
    if(this.domain) {
      ...
      return `${this.domain}${prefix}/${id}`;
    }

    return Meteor.absoluteUrl(`${this.prefix}/${id}`, urlOptions);

  }

  cleanExpiredFromDatabase() {
    const now = new Date();

    this.log(`Removing expired short links from collection at ${now}`);

    const countRemoved = this.shortLinks.remove({
    ...
    this.log(`Removed ${countRemoved} short link(s) from collection`);
  }
